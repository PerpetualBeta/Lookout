...
struct LookoutPanel: View {
        ...
        HStack(spacing: 8) {
            ...
            case .ok(_, let when):
                TimelineView(.periodic(from: .now, by: 1)) { ctx in
                    Text(nextCheckLabel(at: ctx.date))
                        .font(.caption)
    ...
    /// A countdown to the next check — which is what this line was always trying to be.
    ///
    /// It read "in 0 sec" forever. The word "in" was the tell: that is countdown phrasing, and the value
    /// it counted toward was the wrong date entirely — `lastUpdated`, formatted relatively at the instant
    /// the poll had just set it, so the answer was always zero. Nothing re-rendered it either, so the zero
    /// then sat there until the next poll produced an identical one.
    ///
    /// Counting toward `nextPollAt` makes the sentence true, and the per-second tick makes it move. The
    /// cadence is GitHub's, not ours — it comes back on every poll — so the loop publishes the deadline
    /// rather than the panel assuming one.
    private func nextCheckLabel(at now: Date) -> String {
        guard let next = core.nextPollAt else { return "Checking…" }
        let remaining = Int(next.timeIntervalSince(now).rounded(.up))
